---
title: Test (draft)
description: Página para este de configuração.
draft: true
---
...
The issue occurs because MDX interprets curly braces `{}` inside regular Markdown text or JSX elements as JavaScript expressions. The fraction `\frac{f(x+h) - f(x)}{h}` contains `{}` characters, which causes the MDX compiler (via Acorn) to try to parse `{f(x+h) - f(x)}` and `{h}` as JS syntax and fail.

### How to Fix

#### Option 1: Wrap math expressions in KaTeX/MathJax delimiters (Recommended)

If your Docusaurus site uses `@docusaurus/remark-plugin-npm2url` / `@docusaurus/theme-mermaid` or math plugins like `remark-math` and `rehype-katex`, wrap the formula in `$` or `$$`:
...
#### Option 2: Escape the curly braces with backslashes

If you are not using a math plugin and want to display literal LaTeX code or text, escape the braces:
...
#### Option 3: Pass string literals inside JSX braces

You can wrap the raw text inside JavaScript string templates within JSX:
...
In Docusaurus/MDX, the usual way is to specify the **shell/language** after the opening code fence. This gives syntax highlighting, but it doesn't automatically execute the code.
...
If you don't care whether the reader uses Bash, PowerShell, or CMD:
...
But if you want to make it **explicit where the command should be executed**, I recommend putting the environment in the title:

````md
```powershell title="PowerShell — Windows"
cd C:\Users\Adriano\project
npm run dev
```
````
...
Docusaurus supports the `title="..."` code-block metadata, so this is particularly useful for teaching material.
...
**My recommendation for your Docusaurus educational docs:** use the language identifier (`powershell`, `bash`, `cmd`, `python`, `perl`, etc.) **plus `title="..."` when the execution environment matters**. This makes it immediately clear to students _where_ to run the code.
